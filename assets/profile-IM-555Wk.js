import{a as e,i as t,n,t as r}from"./navbar-DS3aPAxg.js";async function i(n){let r=window.localStorage.getItem(`accessToken`),i=window.localStorage.getItem(`apiKey`);if(!r||!i)throw Error(`You must be logged in to view your bids.`);let a=`${t}${e.auction.profiles}/${n}/bids?_listings=true`,o=await fetch(a,{headers:{Authorization:`Bearer ${r}`,"X-Noroff-API-Key":i}}),s=await o.json();if(!o.ok){let e=s.errors?.[0]?.message||`Unable to load your bids.`;throw Error(e)}return s.data}async function a(n,r){let i=window.localStorage.getItem(`accessToken`),a=window.localStorage.getItem(`apiKey`);if(!i||!a)throw Error(`You must be logged in to update your profile.`);let o=`${t}${e.auction.profiles}/${n}`,s=await fetch(o,{method:`PUT`,headers:{Authorization:`Bearer ${i}`,"X-Noroff-API-Key":a,"Content-Type":`application/json`},body:JSON.stringify(r)}),c=await s.json();if(!s.ok){let e=c.errors?.[0]?.message||`Unable to update profile.`;throw Error(e)}return c.data}async function o(){let e=document.querySelector(`#profile-page`);if(!e)return;let t=window.localStorage.getItem(`user`);if(!t){window.location.href=`/login.html`;return}let r=JSON.parse(t);try{let[t,a]=await Promise.all([n(r.name),i(r.name)]);s(e,t,a)}catch(t){console.error(`Failed to load profile:`,t),e.innerHTML=`
      <p class="py-10 text-center text-red-600">
        Failed to load profile.
      </p>
    `}}function s(e,t,n=[]){let r=t.avatar?.url||`/src/assets/images/placeholder-image.png`,i=t.banner?.url||``,a=t.bio||`No bio added yet.`,o=t.credits??0,s=t.listings||[],u=t._count?.listings??s.length,m=d(n);e.innerHTML=`
  ...
  `,c(e,t),l(e)}function c(e,t){let n=e.querySelector(`#edit-profile-button`),r=e.querySelector(`#edit-profile-section`),i=e.querySelector(`#cancel-profile-edit`),a=e.querySelector(`#edit-profile-form`);n?.addEventListener(`click`,()=>{r?.classList.remove(`hidden`)}),i?.addEventListener(`click`,()=>{r?.classList.add(`hidden`)}),a?.addEventListener(`submit`,e=>u(e,t.name))}function l(e){let t=e.querySelector(`#my-listings-tab`),n=e.querySelector(`#my-bids-tab`),r=e.querySelector(`#my-listings-content`),i=e.querySelector(`#my-bids-content`);t?.addEventListener(`click`,()=>{r?.classList.remove(`hidden`),i?.classList.add(`hidden`),t.classList.add(`border-blue-600`,`text-blue-600`),t.classList.remove(`border-transparent`,`text-gray-500`),n?.classList.remove(`border-blue-600`,`text-blue-600`),n?.classList.add(`border-transparent`,`text-gray-500`)}),n?.addEventListener(`click`,()=>{i?.classList.remove(`hidden`),r?.classList.add(`hidden`),n.classList.add(`border-blue-600`,`text-blue-600`),n.classList.remove(`border-transparent`,`text-gray-500`),t?.classList.remove(`border-blue-600`,`text-blue-600`),t?.classList.add(`border-transparent`,`text-gray-500`)})}async function u(e,t){e.preventDefault();let n=e.currentTarget,r=n.querySelector(`#profile-bio`)?.value.trim()||``,i=n.querySelector(`#profile-avatar`)?.value.trim()||``,o=n.querySelector(`#profile-banner`)?.value.trim()||``,s=n.querySelector(`#profile-error`),c=n.querySelector(`#save-profile-button`);g(s);let l={bio:r,avatar:i?{url:i,alt:`${t}'s avatar`}:null,banner:o?{url:o,alt:`${t}'s banner`}:null};c.disabled=!0,c.textContent=`Saving...`;try{await a(t,l),window.location.reload()}catch(e){console.error(`Failed to update profile:`,e),h(s,e.message||`Unable to update profile.`),c.disabled=!1,c.textContent=`Save Profile`}}function d(e=[]){let t=e.map(e=>e.listing).filter(e=>e?.id);return Array.from(new Map(t.map(e=>[e.id,e])).values())}function f(e=[]){return e.length===0?`
      <p class="text-gray-500">
        You have no listings yet.
      </p>
    `:e.map(m).join(``)}function p(e=[]){return e.length===0?`
      <p class="text-gray-500">
        You have not placed any bids yet.
      </p>
    `:e.map(m).join(``)}function m(e){return`
    <article
  ...
  `}function h(e,t){e&&(e.textContent=t,e.classList.remove(`hidden`))}function g(e){e&&(e.textContent=``,e.classList.add(`hidden`))}r(),o();
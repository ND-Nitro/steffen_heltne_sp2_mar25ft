import{a as e,i as t,o as n,t as r}from"./navbar-DtWy4hFO.js";import{t as i}from"./getListing-CIhLswkE.js";async function a(n,r){let i=window.localStorage.getItem(`accessToken`),a=window.localStorage.getItem(`apiKey`);if(!i||!a)throw Error(`You must be logged in to update a listing.`);let o=`${t}${e.auction.listings}/${n}`,s=await fetch(o,{method:`PUT`,headers:{Authorization:`Bearer ${i}`,"X-Noroff-API-Key":a,"Content-Type":`application/json`},body:JSON.stringify(r)}),c=await s.json();if(!s.ok){let e=c.errors?.[0]?.message||`Unable to update listing.`;throw Error(e)}return c.data}async function o(){let e=document.querySelector(`#edit-listing-page`);if(!e)return;let t=window.localStorage.getItem(`user`);if(!t){window.location.href=n(`login.html`);return}let r=JSON.parse(t),a=new URLSearchParams(window.location.search).get(`id`);if(!a){l(e,`Listing ID is missing.`);return}try{let t=await i(a);if(t.seller?.name!==r.name){l(e,`You can only edit your own listings.`);return}s(e,t)}catch(t){l(e,t.message||`Unable to load listing.`)}}function s(e,t){let r=t.media?.[0]?.url||``,i=t.tags?.[0]||``;e.innerHTML=`
  ...
  `,e.querySelector(`#edit-listing-form`)?.addEventListener(`submit`,e=>c(e,t.id))}async function c(e,t){e.preventDefault();let r=e.currentTarget,i=r.querySelector(`#listing-title`)?.value.trim()||``,o=r.querySelector(`#listing-description`)?.value.trim()||``,s=r.querySelector(`#listing-image`)?.value.trim()||``,c=r.querySelector(`#listing-category`)?.value.trim()||``,l=r.querySelector(`#edit-listing-error`),d=r.querySelector(`#save-listing-button`);if(!i){u(l,`Title is required.`);return}let f={title:i,description:o,tags:c?[c]:[],media:s?[{url:s,alt:i}]:[]};d.disabled=!0,d.textContent=`Saving...`;try{await a(t,f),window.location.href=n(`listing.html?id=${t}`)}catch(e){u(l,e.message||`Unable to update listing.`),d.disabled=!1,d.textContent=`Save Changes`}}function l(e,t){e.innerHTML=`

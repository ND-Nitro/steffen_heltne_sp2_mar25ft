import{a as e,i as t,o as n,t as r}from"./navbar-Aux6msdz.js";async function i(n){let r=window.localStorage.getItem(`accessToken`),i=window.localStorage.getItem(`apiKey`);if(!r||!i)throw Error(`You must be logged in to create a listing.`);let a=`${t}${e.auction.listings}`,o=await fetch(a,{method:`POST`,headers:{Authorization:`Bearer ${r}`,"X-Noroff-API-Key":i,"Content-Type":`application/json`},body:JSON.stringify(n)}),s=await o.json();if(!o.ok){let e=s.errors?.[0]?.message||`Unable to create listing.`;throw Error(e)}return s.data}function a(){let e=document.querySelector(`#create-listing-page`);if(e){if(!window.localStorage.getItem(`user`)){window.location.href=n(`login.html`);return}o(e)}}function o(e){e.innerHTML=`
  ...
  `,e.querySelector(`#create-listing-form`)?.addEventListener(`submit`,s)}async function s(e){e.preventDefault();let t=e.currentTarget,n=t.querySelector(`#listing-title`)?.value.trim()||``,r=t.querySelector(`#listing-description`)?.value.trim()||``,a=[t.querySelector(`#listing-image-1`)?.value.trim()||``,t.querySelector(`#listing-image-2`)?.value.trim()||``,t.querySelector(`#listing-image-3`)?.value.trim()||``].filter(Boolean),o=t.querySelector(`#listing-category`)?.value||``,s=t.querySelector(`#listing-deadline`)?.value||``,d=t.querySelector(`#create-listing-error`),f=t.querySelector(`#create-listing-button`);if(l(d),!n||!s){c(d,`Title and deadline are required.`);return}let p=new Date(s).toISOString();if(new Date(p).getTime()<=Date.now()){c(d,`Deadline must be in the future.`);return}let m={title:n,endsAt:p};r&&(m.description=r),o&&(m.tags=[o]),a.length>0&&(m.media=a.map((e,t)=>({url:e,alt:`${n} image ${t+1}`}))),u(f,!0);try{let e=await i(m);window.location.href=`/listing.html?id=${e.id}`}catch(e){c(d,e.message||`Unable to create listing.`)}finally{u(f,!1)}}function c(e,t){e&&(e.textContent=t,e.classList.remove(`hidden`))}function l(e){e&&(e.textContent=``,e.classList.add(`hidden`))}function u(e,t){e&&(e.disabled=t,e.textContent=t?`Publishing...`:`Publish`)}r(),a();